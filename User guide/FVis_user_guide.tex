...
\begin{itemize}
	...
	\item \ttt{\cro{backgrounds}=\cbl{None}}: Type: \ptype{dict}.\\Used for specifying alternative initial states $\phi(\mathbf{r}, 0)$ to use when visualising any of the contrast quantities \hyperref[sec:quants]{here}. If set to \ttt{\cbl{None}}, the earliest state in the current set of visualisation data will be used. Otherwise, the argument must be a dictionary where the keys are the names of the relevant quantities (like \ttt{\cyo{'rho'}}, \ttt{\cyo{'T'}} etc.), and the values are 2D arrays representing the corresponding initial states.
